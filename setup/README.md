...
- **Roles** (padrão CIn `sys_<realm>-<perfil>`, iguais às do frontend):
  `sys_chatbot-admin`, `sys_chatbot-users`
- **Usuários de teste**:
  | Usuário | Senha | Roles |
  |---|---|---|
  | `admin` | `admin` | `sys_chatbot-admin`, `sys_chatbot-users` |
  | `user` | `user` | `sys_chatbot-users` |
- **Clients**:
  | Client ID | Tipo | Uso |
  |---|---|---|
  | `chatbot-back` | confidential, service account | API (backend), secret `dev-secret` |
  | `chatbot-front` | public | SPA/frontend, sem secret |
...
`chatbot` vira o nome do projeto informado na criação — no `id` do realm, nos
`clientId` dos dois clients, nas roles do sistema (`sys_chatbot-admin`,
`sys_chatbot-users`) e nas roles padrão (`default-roles-chatbot`). O campo
