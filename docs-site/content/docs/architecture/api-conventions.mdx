...
- **Prefixo Global**: Todas as rotas de negócio estão sob `/<nome-do-projeto>/api/v1` — o segmento `<nome-do-projeto>/api` é configurado em `src/main.ts` via `app.setGlobalPrefix(configService.get('API_PREFIX'))`, e o `v1` vem do versionamento URI (`app.enableVersioning`). O valor de `API_PREFIX` fica em `.env` (`API_PREFIX=chatbot/api` no projeto gerado, ex: `meu-servico/api`).
